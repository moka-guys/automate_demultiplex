...
temp_samplesheet_path = os.path.join(
    temp_samplesheets_dir, "%s_SampleSheet.csv"
    )
...
LOGDIRS = {
    ...
    "backup_runfolder": os.path.join(
        temp_log_dir, "backup_runfolder_logfiles"),
    "upload_script": os.path.join(
        temp_log_dir, "upload_agent_script_logfiles"),
...
LOGFILES = {
    ...
    "demultiplex_script_logfile": os.path.join(
        temp_log_dir, "Demultiplexing_log_files/",
    ...
    "proj_creation_script": os.path.join(
        temp_log_dir, "nexus_project_creation_scripts",
...
def run_before_and_after_tests(monkeypatch):
    ...
    monkeypatch.setattr(runfolder_obj.ad_config, "SAMPLESHEET_PATH", temp_samplesheet_path)
