...
1. Identify runfolders in the runfolders directory which have not been processed:
    - Runfolder contains bclconvert2 log file with success string
...
5. Generate the pre-pipeline upload commands (cluster density files, bclconvert2 QC files, logfiles, fastqs if not a tso run, SampleSheets, MasterFile if an oncodeep run)
