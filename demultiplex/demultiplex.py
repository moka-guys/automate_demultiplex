...
class DemultiplexRunfolder(DemultiplexConfig):
    """
    Call bcl2fastq2 or bases2fastq on runfolders after asserting that 
    runfolder has not been demultiplexed and a valid SampleSheet is present.
    ...
    Attributes
        ...
        demux_rf_logger (object):           Demultiplex runfolder-level logger, extracted from
                                            ...
                                            loggers
        demultiplex_rf_logger (object):      demultiplex runfolder-level logger, extracted from the
                                            RunfolderObject containing runfolder-level loggers
        demultiplex_cmd (str):               Shell command to run demultiplexing
        cluster_density_cmd (str):          Shell command to run cluster density calculation (Illiumina only)
    ...
    Methods
        ...
        create_demultiplex_log()
            Create file to prevent demultiplexing starting again (bcl2fastq & bases2fastq)
    ...
    def setoff_workflow(self) -> Optional[bool]:
        ...
        next run of the script (bcl2fastq2 & bases2fastq are slow to create the logfile). Then calls
        calculate_cluster_density() if an Illumina run. If a tso run, stops here. Else calls
    ...
    def runtype_requires_demultiplexing(self) -> Optional[bool]:
        ...
        if any(
            ...
            self.create_demultiplex_log()  # Create bcl2fastq2 log to prevent scripts processing this run
