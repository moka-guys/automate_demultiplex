...
class DemultiplexRunfolder(DemultiplexConfig):
    ...
    def run_demultiplexing(self) -> Optional[bool]:
        ...
        if returncode == 0:
            failed_fastqs = validate_fastqs(self.rf_obj.fastq_dir_path, self.demux_rf_logger)
            self.rf_obj.failed_fastqs = failed_fastqs
            if failed_fastqs:
                self.demux_rf_logger.warning(
                    self.demux_rf_logger.log_msgs["fastqs_failed_warning"],
                    ", ".join(failed_fastqs),
                )
            self.demux_rf_logger.info(
